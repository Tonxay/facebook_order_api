...
func Login(c *fiber.Ctx) error {
	...
	accessToken, _ := middleware.GenerateToken(user.ID, user.Rolo, time.Hour)
	refreshToken, _ := middleware.GenerateToken(user.ID, user.Rolo, (23*time.Hour)*2)
...
func Refresh(c *fiber.Ctx) error {
	...
	newAccessToken, _ := middleware.GenerateToken(userID, role, time.Hour)
	refreshToken, _ := middleware.GenerateToken(userID, role, (23*time.Hour)*2)
	return c.JSON(fiber.Map{
