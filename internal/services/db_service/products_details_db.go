...
func GetProductDetailsForID(db *gorm.DB, productItemDetailId string, size_id string) (custommodel.ProductDetailCounter, error) {
	...
	tx = tx.Select("pd.id,p.name, s.size, pd.color, SUM(spd.remaining) AS quantity,s.price ")
	...
	tx = tx.Group("pd.id,p.name, s.size, pd.color,s.price ")
...
func GetProductByIDSizdID(db *gorm.DB, productItemDetailId, sizeId, producId string) (custommodel.ProductOrderDetails, error) {
	...
	tx = tx.Select("pd.product_id, pd.id, p.name, s.id  AS size_id , s.size ,pd.color_name")
	...
	tx = tx.Group("pd.product_id,pd.id,p.name, s.size, pd.color_name,s.id")
